...
public class DirectoryRepositoryRaw implements ContactRepository, DirectoryChangeListener {
	...
	public DirectoryRepositoryRaw(String dirName, ServiceTracker listenerTracker, LogService log) {
	...
	public void filesAdded(File[] addedFiles) {
		...
		for (Object listener : listeners) {
			try {
				((ContactRepositoryListener) listener).contactsAdded(newContacts);
			} catch(Exception e) {
				// Ignore
			}
		}
	...
	public void filesRemoved(File[] removedFiles) {
		...
		for (Object listener : listeners) {
			try {
				((ContactRepositoryListener) listener).contactsRemoved(removedContacts);
			} catch(Exception e) {
				// Ignore
			}
		}
